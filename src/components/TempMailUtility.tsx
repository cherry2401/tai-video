...
const TempMailUtility: React.FC = () => {
  ...
  const createMailbox = async () => {
    ...
    try {
      ...
      if (!response.ok || !data.email || !data.token || !data.password) {
        throw new Error(data.message || 'Khong the tao email tam.');
      }
      setAccount({
        email: data.email,
        token: data.token,
      ...
      setAccount(null);
      setMessages([]);
      setError(err instanceof Error ? err.message : 'Loi tao mailbox.');
    } finally {
  ...
  const refreshInbox = async () => {
    ...
    try {
      ...
      const data = (await response.json()) as TempMailApiResponse;
      if (!response.ok) {
        throw new Error(data.message || 'Khong the doc hop thu.');
      }
      setMessages(Array.isArray(data.answer) ? data.answer : []);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Loi doc hop thu.');
    } finally {
    ...
    <div className="w-full max-w-4xl mx-auto animate-fadeIn pb-20">
      <div className="text-center mb-7 space-y-2">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm md:text-base">
          Tao email tam de nhan mail nhanh, han che spam vao hop thu chinh.
        </p>
      </div>
          ...
          <div className="min-h-[300px] max-h-[500px] overflow-y-auto">
            {!account && (
              <div className="h-full min-h-[300px] flex items-center justify-center text-center px-6">
                <div>
                  <p className="text-gray-700 dark:text-gray-200 font-semibold">Chua co hop thu tam</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">Bam Tao de tao email tam va bat dau nhan thu.</p>
                </div>
              </div>
            )}
            ...
            {account && loadingInbox && (
              <div className="h-full min-h-[300px] flex items-center justify-center text-center px-6">
                <div>
                  ...
                  <p className="text-gray-700 dark:text-gray-200 font-semibold mt-4">Dang tai thu...</p>
                </div>
              </div>
            )}
            ...
            {account && !loadingInbox && messages.length === 0 && (
              <div className="h-full min-h-[300px] flex items-center justify-center text-center px-6">
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">Dang cho thu den. Bam refresh de kiem tra lai.</p>
                </div>
              </div>
            )}
              ...
              <div className="divide-y divide-gray-200 dark:divide-gray-700">
                {messages.map((mail, index) => (
                  <div key={mail.id || `${index}-${mail.subject || 'mail'}`} className="px-4 md:px-6 py-3.5 hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                    <p className="font-semibold text-gray-900 dark:text-gray-100">{mail.subject || '(Khong co tieu de)'}</p>
                    <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">
                      Tu: {mail.from || 'N/A'} {mail.date || mail.createdAt ? `• ${mail.date || mail.createdAt}` : ''}
                    </p>
                    <p className="text-sm text-gray-700 dark:text-gray-300 mt-2 break-words">{mail.intro || mail.text || 'Khong co noi dung preview.'}</p>
                  </div>
                ))}
              </div>
            )}
