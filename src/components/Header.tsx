...
const Header: React.FC<HeaderProps> = ({
  ...
  const desktopShellClass = theme === 'dark'
    ? 'md:[background-color:rgba(24,22,63,0.34)] md:[background-image:linear-gradient(90deg,rgba(15,23,42,0.42)_0%,rgba(30,27,75,0.34)_45%,rgba(22,30,72,0.42)_100%)] md:border-indigo-300/25 md:shadow-[0_22px_48px_rgba(2,6,23,0.36),inset_0_1px_0_rgba(255,255,255,0.12)]'
  ...
  return (
    <header
      className={`bg-white md:bg-transparent dark:bg-[#18163f] dark:[background-image:linear-gradient(90deg,#0f172a_0%,#1e1b4b_45%,#161e48_100%)] md:dark:bg-transparent md:dark:[background-image:none] text-gray-800 dark:text-white py-3 md:pt-2 md:pb-4 px-4 md:px-6 transition-colors duration-300 sticky top-0 z-50 w-full border-b md:border-b-0 ${
        isMenuOpen
    ...
    >
      <div className="container mx-auto">
        <div className={`flex justify-between items-center md:max-w-[1480px] md:mx-auto md:rounded-[26px] md:border md:backdrop-blur-2xl md:px-6 lg:px-8 md:h-[68px] ${desktopShellClass}`}>
          <div
          ...
          >
            {theme === 'dark' ? (
              <img
                src={`/logo_taivideo4_dark.svg?v=${logoCacheBust}`}
                alt="TaiVideo logo"
                className="h-6 md:h-7 w-auto object-contain shrink-0"
              />
            ) : (
              <img
                src={`/logo_taivideo4.svg?v=${logoCacheBust}`}
                alt="TaiVideo logo"
                className="h-6 md:h-7 w-auto object-contain shrink-0"
              />
            )}
